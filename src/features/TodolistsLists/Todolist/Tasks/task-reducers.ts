...
export const taskReducers = (
  state: TaskStateType = InitialTaskState,
  ...
  switch (action.type) {
    ...
    case REMOVE_TASK:
      return {
        ...state,
        [action.todolistId]: [
          ...state[action.todolistId].filter((tsk) => tsk.id !== action.taskId),
        ],
      };
    case UPDATE_TASK:
      return {
...
export const UpdateTaskAC = (
  todolistId: string,
  taskId: string,
  model: UpdateTaskModelDomainType,
) => {
...
export const deleteTaskTC =
  ({ todolistId, taskId }: DeleteTaskTCArgs): AppThunk =>
  (dispatch) => {
    dispatch(setAppStatusAC("loading"));
...
export const addTaskTC =
  ({ todolistId, title }: CreateTaskTCArgs): AppThunk =>
  (dispatch) => {
    dispatch(setAppStatusAC("loading"));
    todolistApi
...
export const UpdateTaskTC =
  (
    todolistId: string,
    taskId: string,
    domainModel: UpdateTaskModelDomainType,
  ): AppThunk =>
